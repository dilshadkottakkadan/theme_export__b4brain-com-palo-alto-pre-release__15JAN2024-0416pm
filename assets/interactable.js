   ...
   */
    ...
    geckoShopify.currencyForm = function () {
      ...
      geckoShopify.NewsletterPopup = function () {
        ...
        if ($('.popup_new_wrap').length == 0 || ($('.mobile_new_false').length > 0 && $(window).width() < 768) || (Cookies.get('bmax_age_verify') != 'confirmed' && $('.popup_age_wrap').length > 0) ) return;
        var popup = $('.popup_new_wrap'),
          stt = popup.data('stt'),
          pp_version = stt.pp_version,
          shown = false,
          pages = Cookies.get('bmax_shown_pages');
          ...
          var showPopup = function () {
            $.magnificPopup.open({
              ...
              callbacks: {
                ...
                close: function () {
                  Cookies.set('bmax_popup_' + pp_version, 'shown', { expires: stt.day_next, path: '/' });
                }
                // e.t.c.
              }
            });
          ...
          $('.bmax_open_newsletter').on('click', function (e) {
            e.preventDefault();
            showPopup2();
          });
          ...
          if (pages < stt.number_pages) {
            pages++;
            Cookies.set('bmax_shown_pages', pages, { expires: stt.day_next, path: '/' });
            return false;
          }

          if (Cookies.get('bmax_popup_' + pp_version) != 'shown') {
            if (stt.after == 'scroll') {
              $(window).scroll(function () {
      ...
      geckoShopify.ageVerify = function () {
          if ( $('.popup_age_wrap').length == 0 || ( !designMode && Cookies.get('bmax_age_verify') == 'confirmed') ) return;
          ...
          $('.age_verify_allowed').on('click', function(){
            ...
            if (date_of_birth) {
               ...
               if ((today.getTime() - theirDate.getTime()) < 0) {
                ...
                Cookies.set('bmax_age_verify', 'confirmed', { expires: parseInt( day_next ), path: '/' });
                $.magnificPopup.close();
               }
            } else {
              Cookies.set('bmax_age_verify', 'confirmed', { expires: parseInt( day_next ), path: '/' });
      ...
      geckoShopify.cookiesLawPP = function () {
        ...
        if ( (!designMode && Cookies.get('bmax_cookies_' + pp_version) == 'accepted') || popup.length == 0 ) return;
        ...
        var acceptCookies = function () {
          ...
          Cookies.set('bmax_cookies_' + pp_version, 'accepted', { expires: stt.day_next, path: '/' });
        };
      ...
      geckoShopify.PromoPrPopup = function () {
        var pp_version = 1;
        if ($('.js_lz_pppr').length == 0 || window_w < 1025 || !yesHover || (Cookies.get('bmax_age_verify') != 'confirmed' && $('.popup_age_wrap').length > 0) || (!designMode && Cookies.get('bmax_prpr_pp_' + pp_version) == 'shown')) return;
          ...
          var showPopup = function () {
            ...
            $.magnificPopup.open({
              ...
              callbacks: {
                ...
                close:function () {
                    Cookies.set('bmax_prpr_pp_' + pp_version, 'shown', { expires: stt.day_next, path: '/' });
                }
                // e.t.c.
              }
            });
          ...
          $('.bmax_open_promopr').on('click', function (e) {
            e.preventDefault();
            showPopup();
          });
      ...
      geckoShopify.SalesPopup = function () {
        ...
        if ($('.popup_slpr_wrap').length == 0 || ($('.salse_pp_mb_false').length > 0 && $(window).width() < 768) || (Cookies.get('bmax_age_verify') != 'confirmed' && $('.popup_age_wrap').length > 0)) return;
